...
ImageUpload.prototype.insert = function (data, callback) {
        ...
        var gridStore = new GridStore(this.db,fileId , 'w',{ root:"image","content_type":data.type,"chunk_size":data.length});
...
ImageUpload.prototype.read = function (fileId, callback) {
    var gridStore = new GridStore(this.db, new ObjectID(fileId),'r',{root:'image'});
    gridStore.open(function (err, gridStore) {
        if(err){
            //console.log(err);
            callback(err);
            ...
            gridStore.seek(0, function() {
                ...
                gridStore.read(function(err,data) {
                    callback(gridStore.contentType,data)
                });
            })
        }
    });
...
ImageUpload.prototype.delete = function(fileId){
    var gridStore = new GridStore(this.db, new ObjectID(fileId),'r',{root:'image'});
    gridStore.open(function (err, gridStore) {
        if(err){
            //console.log(err);
